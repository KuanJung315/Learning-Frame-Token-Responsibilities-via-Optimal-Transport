...
def get_parser():
    ...
    parser.add_argument(
        "--ctc-nonblank-logit-bias",
        type=float,
        default=0.0,
        help=(
            "Decode-time CTC calibration. This value is added to every "
            "non-blank token logit and the distribution is re-normalized. "
            "Positive values increase non-blank probability relative to blank."
        ),
    )
    parser.add_argument(
        "--ctc-blank-logit-bias",
        type=float,
        default=0.0,
        help=(
            "Decode-time CTC calibration for the blank logit. This is mostly "
            "for diagnostics; --ctc-nonblank-logit-bias is the direct control "
            "matching VI's positive prior-logit-bias sweep."
        ),
    )
...
def _format_bias_for_key(value: float) -> str:
    return f"{value:g}"


def _ctc_bias_key_suffix(params: AttributeDict) -> str:
    suffix = (
        f"_ctc-nb-bias-"
        f"{_format_bias_for_key(params.ctc_nonblank_logit_bias)}"
    )
    if params.ctc_blank_logit_bias != 0.0:
        suffix += (
            f"_ctc-blank-bias-"
            f"{_format_bias_for_key(params.ctc_blank_logit_bias)}"
        )
    return suffix


def apply_ctc_logit_bias(log_probs: torch.Tensor, params: AttributeDict) -> torch.Tensor:
    """Apply a blank/non-blank logit calibration to CTC log-probabilities."""
    nonblank_bias = params.ctc_nonblank_logit_bias
    blank_bias = params.ctc_blank_logit_bias
    if nonblank_bias == 0.0 and blank_bias == 0.0:
        return log_probs

    biased = log_probs.clone()
    if blank_bias != 0.0:
        biased[..., 0] = biased[..., 0] + blank_bias
...
def decode_one_batch(
    ...
    nnet_output = apply_ctc_logit_bias(nnet_output, params)
    key_suffix = _ctc_bias_key_suffix(params)
    ...
    if params.method == "nbest-oracle":
        ...
        key = (
            f"oracle_{params.num_paths}_nbest_scale_{params.nbest_scale}"
            f"{key_suffix}"
        )
        return {key: hyps}
    ...
    if params.method in ["1best", "nbest"]:
        if params.method == "1best":
            ...
            key = (
                f"no_rescore-nbest-scale-{params.nbest_scale}-{params.num_paths}"
                f"{key_suffix}"
            )
    ...
    if best_path_dict is not None:
        for lm_scale_str, best_path in best_path_dict.items():
            hyps = get_texts(best_path)
            hyps = [[word_table[i] for i in ids] for ids in hyps]
            ans[f"{lm_scale_str}{key_suffix}"] = hyps
    else:
        ans = None
    return ans
