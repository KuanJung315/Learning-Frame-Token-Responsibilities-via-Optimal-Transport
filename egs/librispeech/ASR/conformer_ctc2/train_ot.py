...
"""
CTC + OT (Optimal Transport) alignment loss training on conformer_ctc2.
...
./conformer_ctc2/train_ot.py \
  --world-size 4 \
  --num-epochs 30 \
  --start-epoch 1 \
  --exp-dir conformer_ctc2/exp_ot \
  --full-libri 1 \
  --att-rate 0.0 \
...
def _compute_bpe_lengths(token_ids: list, sp, device: torch.device) -> Tensor:
    """Return per-token character counts for the OT column marginal."""
    lengths = []
...
def get_parser():
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.15,
        help="Width of the soft diagonal mask used for acoustic token marginals.",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.0,
        help="Temperature for acoustic scores when building acoustic token marginals.",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.05,
        help="Uniform floor mixed into column marginals to avoid zero-mass tokens.",
    )

    parser.add_argument(
        "--ot-col-marginal-type",
        type=str,
        default="bpe",
        choices=["bpe", "acoustic"],
        help=(
            "'bpe': column marginal proportional to BPE token character length. "
            "'acoustic': previous soft diagonal/acoustic token prior."
        ),
    )
...
def compute_loss(
    ...
    if lambda_ot > 0 and isinstance(graph_compiler, BpeCtcTrainingGraphCompiler):
        ...
        # nnet_output is NOT sorted (it's in original batch order).
        # supervision_segments[:, 0] gives the original sequence index for
        # each sorted position. We need to map sorted index -> original index.
        ...
        for sorted_idx, ids in enumerate(token_ids):
            ...
            bpe_lengths = None
            if params.ot_col_marginal_type == "bpe":
                bpe_lengths = _compute_bpe_lengths(ids, graph_compiler.sp, lp.device)
            blank_id = 0

            if debug and sorted_idx == 0:
                loss_i, P, keep = ot_prior_loss_from_logprobs(
                    log_probs=lp,
                    labels=labels,
