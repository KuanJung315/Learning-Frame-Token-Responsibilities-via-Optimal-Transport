...
def gate_log_probs_with_blank_prior(
    log_probs: torch.Tensor,
    alpha: torch.Tensor,
    eps: float = 1e-5,
) -> torch.Tensor:
    """
    Replace the CTC blank/non-blank mass using the learned variational prior.

    alpha is p(non-blank | X) from BlankPriorHead.  We set:
      p(blank | t) = 1 - alpha_t
      p(k != blank | t) = alpha_t * p_ctc(k | t) / sum_j!=blank p_ctc(j | t)
    """
    ...
    log_nonblank = (
        ...
        + log_probs[:, :, 1:]
        - log_nonblank_sum.unsqueeze(-1)
    )

    return torch.cat([log_blank.unsqueeze(-1), log_nonblank], dim=-1)


class ConformerVIForDecode(nn.Module):
    """
    Inference wrapper for checkpoints trained by train_vi_ot.py.
    ...
    blank_prior predicts p(non-blank | X) from encoder states and gates the CTC
    log-probs before lattice/greedy decoding.
    """
    ...
    def forward(self, x, supervisions=None, warmup: float = 1.0):
        log_probs, memory, memory_key_padding_mask = self.encoder(
            x, supervisions, warmup=warmup
        )

        encoder_out = memory.permute(1, 0, 2)  # (T, N, C) -> (N, T, C)
        if memory_key_padding_mask is None:
            encoder_out_lens = torch.full(
                (encoder_out.size(0),),
                encoder_out.size(1),
                dtype=torch.int64,
            ...
            encoder_out_lens = (~memory_key_padding_mask).sum(dim=1).to(torch.int64)

        alpha_prior = self.blank_prior(encoder_out, encoder_out_lens)
        log_probs = gate_log_probs_with_blank_prior(log_probs, alpha_prior)
...
def main():
    ...
    blank_gate = BlankGateHead(d_model=params.encoder_dim, vocab_size=num_classes)
    blank_prior = BlankPriorHead(d_model=params.encoder_dim)
    model = ConformerVIForDecode(conformer, blank_gate, blank_prior)
