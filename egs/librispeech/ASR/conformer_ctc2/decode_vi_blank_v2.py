...
def decode_dataset(
    ...
    for batch_idx, batch in enumerate(dl):
        ...
        except Exception as e:  # flaky k2 lattice/OOM blow-ups should not kill the run
            logging.warning(f"decode_one_batch failed on batch {batch_idx}: {e}")
        ...
        if hyps_dict is not None:
            ...
            # First batch produced nothing and no keys exist yet to attach empty
            # hyps to; skip it rather than aborting the whole decode.
            logging.warning(f"batch {batch_idx} decoded to empty with no keys yet; skipping")
        else:
            # Count the failed batch's utterances as empty hyps (deletions) so the
            # WER stays honest instead of silently dropping hard utterances.
            this_batch = []
            hyp_words = []
