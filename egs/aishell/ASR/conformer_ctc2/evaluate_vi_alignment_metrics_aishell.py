...
def get_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--baseline-epoch", type=int, default=90)
    parser.add_argument("--baseline-avg", type=int, default=20)
    ...
    parser.add_argument(
        ...
        default=0.0,
        help=(
            "Optional baseline calibration: add this value to all non-blank "
            "log-probs and renormalize before metric computation."
        ),
    )

    parser.add_argument("--candidate-exp-dir", type=Path, required=True)
    parser.add_argument("--candidate-name", type=str, default="vi_ot_v2")
    parser.add_argument("--candidate-epoch", type=int, default=63)
    parser.add_argument("--candidate-avg", type=int, default=20)
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("conformer_ctc2/alignment_eval_vi_v2_aishell"),
    )
...
def _params_from_checkpoint(exp_dir: Path, epoch: int) -> AttributeDict:
...
def _build_baseline_model(
    ...
    params = _params_from_checkpoint(args.baseline_exp_dir, args.baseline_epoch)
...
def _build_candidate_model(
    ...
    params = _params_from_checkpoint(args.candidate_exp_dir, args.candidate_epoch)
    if not hasattr(params, "num_decoder_layers"):
...
def _load_eval_dataloader(args: argparse.Namespace):
    ...
    datamodule = AishellAsrDataModule(args)
    if args.dataset == "dev":
...
def _batch_log_probs(
    ...
    with torch.no_grad():
        log_probs, _, memory_key_padding_mask = model(feature, supervisions, warmup=1.0)
    ...
    for i in range(log_probs.size(0)):
        output_len = max(int(output_lens[i].item()), 1)
...
def _collect_rows(
    ...
    for batch_idx, batch in enumerate(dl):
        ...
        for i, text in enumerate(texts):
            ...
            pieces = [graph.token_table[token_id] for token_id in token_ids[i]]
...
def main() -> None:
    ...
    logging.info("AISHELL alignment evaluation started")
    logging.info(args)
    ...
    prefix = (
        args.output_dir
        / f"{args.dataset}_"
        f"base_e{args.baseline_epoch}_avg{args.baseline_avg}_"
        ...
        f"bias{_bias_tag(args.candidate_prior_logit_bias)}_"
        f"maxcuts{args.max_cuts}"
    )
