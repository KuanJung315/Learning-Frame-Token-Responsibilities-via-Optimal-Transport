...
def vi_ot_loss_v2_batched(
    ...
    # Row marginal from the (detached) blank gate alpha.
    a = _masked_normalize(alpha.detach().float().clamp_min(tiny), row_mask, tiny)
    ...
    if column_marginal_type == "bpe":
        ...
        if t_frac is None:
            t_frac = _frac_positions(frame_lens, T, device)
            u_frac = _frac_positions(label_lens, U, device)
        scores = (-C_detached / temp).masked_fill(~col_mask[:, None, :], _NEG)
    ...
    # Batched Sinkhorn on the detached cost (with NEG-masked padding).
    log_K = -C_detached / max(float(eps), tiny)
