...
def get_parser():
    ...
    parser.add_argument("--lang-dir", type=str, default="data/lang_bbpe_500")
    parser.add_argument(
        "--max-train-cuts",
        type=int,
        default=0,
...
def compute_loss(
    ...
    if not isinstance(
        ...
        raise ValueError("VarCTC v2 currently requires BPE or char targets.")
...
def run(rank: int, world_size: int, args) -> None:
    ...
    if "lang_bpe" in str(params.lang_dir) or "lang_bbpe" in str(params.lang_dir):
        ...
        raise ValueError("VarCTC v2 requires a lang_bpe/lang_bbpe/lang_char directory.")
