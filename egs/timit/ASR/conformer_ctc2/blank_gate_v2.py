...
class BlankGateHeadV2(nn.Module):
    """
    Cross-attention posterior non-blank gate q_psi(z_nb^t = 1 | X, Y).
    ...
    def __init__(
        ...
        # Kept as init_blank_prob for checkpoint/CLI compatibility; alpha is the
        # non-blank probability used by build_gated_log_probs_v2.
...
class BlankPriorHeadV2(nn.Module):
    """
    Prior p_beta(z_nb^t = 1 | X) for the non-blank gate.
