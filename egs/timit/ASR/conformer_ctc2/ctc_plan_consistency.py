...
def ctc_token_occupancy_batched(
    ...
    # A two-state skip is legal only when entering a non-blank state whose
    # label differs from the label two states back.
    ...
    # Forward recursion. Lists avoid indexed writes that can invalidate the
    # autograd graph.
    ...
    # Backward recursion with per-utterance terminal frames.
...
def temporal_plan_w1_loss(
    ...
    # Do not divide by utterance length: the result is deliberately measured
    # in encoder-frame units and remains comparable across frame rates.
