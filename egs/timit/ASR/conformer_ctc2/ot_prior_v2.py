...
def vi_ot_loss_v2(
    ...
    C_plan_detached = C_acoustic_detached
    ...
    if beta_pos > 0:
        ...
        C_plan_detached = C_plan_detached + pos_cost
    ...
    P = _sinkhorn(a, b, C_plan_detached, eps=eps, iters=iters)
    loss = (P.detach() * C).sum()
...
def vi_ot_loss_v2_batched(
    ...
    # The standard OT loss keeps the plan detached. A separate consistency
    # objective can request a differentiable plan so timing gradients reach
    # the gate and acoustic cost without changing the original OT loss.
    ...
    if column_marginal_type == "bpe":
        ...
        if t_frac is None:
            t_frac = _frac_positions(frame_lens, T, device)
            u_frac = _frac_positions(label_lens, U, device)
        scores = (-C_acoustic_detached / temp).masked_fill(
            ~col_mask[:, None, :], _NEG
        )
    ...
    # Batched Sinkhorn (with NEG-masked padding).
