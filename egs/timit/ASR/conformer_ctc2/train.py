...
./conformer_ctc2/train.py \
  ...
  --num-epochs 30 \
  --start-epoch 1 \
  --exp-dir conformer_ctc2/exp_ctc2_baseline \
...
def get_parser():
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.0,
        help="Learning-rate multiplier for the pretrained acoustic backbone.",
    )
    parser.add_argument(
        ...
        default=None,
        help=(
            "Initialize subsampling and Conformer encoder blocks from a "
            "VFTA checkpoint while reinitializing the target-dependent head."
        ),
    )
...
def compute_loss(
    ...
    model: Union[nn.Module, DDP],
    batch: dict,
    graph_compiler: Union[BpeCtcTrainingGraphCompiler, CharCtcTrainingGraphCompiler],
    is_training: bool,
    warmup: float = 1.0,
    ...
    if isinstance(
        ...
        # Works with a BPE or char model
...
def initialize_from_pretrained_encoder(
    model: nn.Module, filename: Path
) -> None:
    """Load the tokenization-independent Conformer backbone from VFTA."""
    ...
    for target_name, target_value in target.items():
        # Optional objective-specific state belongs to the TIMIT model itself,
        # not to the tokenization-independent Conformer backbone being loaded.
        # In particular, LabelPriorConformer registers these buffers at the
        # top level, so attempting to find ``encoder.label_prior`` in a VFTA
        # checkpoint would incorrectly reject an otherwise compatible model.
        if target_name in {"label_prior", "label_prior_ready"}:
            continue
        if target_name.startswith("encoder_output_layer.") or target_name.startswith(
            "decoder."
        ):
            continue
        source_name = f"encoder.{target_name}"
        if source_name not in source or source[source_name].shape != target_value.shape:
            incompatible.append((target_name, source_name))
            continue
        transferred[target_name] = source[source_name]
    if incompatible:
        preview = ", ".join(
            f"{target}<-{source_name}" for target, source_name in incompatible[:8]
        )
        raise ValueError(
            "Pretrained VFTA encoder is incompatible with vanilla CTC. "
            f"First keys: {preview}"
        )
    model.load_state_dict(transferred, strict=False)
    num_parameters = sum(value.numel() for value in transferred.values())
    logging.info(
        "Initialized %d tensors (%.2fM parameters) from %s",
        len(transferred),
...
def run(rank, world_size, args):
    ...
    if "lang_bpe" in str(params.lang_dir) or "lang_bbpe" in str(params.lang_dir):
        ...
        raise ValueError(
            ...
            f"'lang_bpe', 'lang_bbpe', 'lang_char', or 'lang_phone' in its name): "
            f"{params.lang_dir}"
        )
    ...
    if params.init_encoder_checkpoint is not None:
        if params.start_epoch > 1 or params.start_batch > 0:
            raise ValueError(
                "--init-encoder-checkpoint cannot be combined with resume options"
            )
        initialize_from_pretrained_encoder(
            model, filename=params.init_encoder_checkpoint
        )
    ...
    if params.encoder_lr_scale == 1.0:
        ...
        for name, parameter in model.named_parameters():
            if name.startswith("encoder_output_layer.") or name.startswith(
                "decoder."
            ):
        ...
        optimizer_parameters = [
            ...
            {"params": head_parameters, "lr": params.initial_lr},
        ]
        logging.info(
            "Using differential learning rates: encoder=%.6g, head=%.6g",
            params.initial_lr * params.encoder_lr_scale,
            params.initial_lr,
        )
    optimizer = Eve(optimizer_parameters, lr=params.initial_lr)
